...
import { getTransferRequest, approveTransfer, declineTransfer} from "../../../api/api"
    ...
    const handleApprove =  (reqID)=>{
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                let date = new Date((new Date()).getTime() + (day * 86400000))
                  let data = {leavingDate:date, reqID: reqID}
                  approveTransfer(data).then((res)=> {
                    getTransferRequest().then(res=> setAllRequest(res.data))
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const { value: text } = await Swal.fire({
                    input: 'textarea',
                    inputLabel: 'Reason for rejecting Transfer Request',
